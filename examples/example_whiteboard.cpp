...
int main(int, char **)
{
    ...
    float main_scale = ImGui_ImplGlfw_GetContentScaleForMonitor(monitor); // Valid on GLFW 3.3+ only

    std::cout << "Screen resolution: " << mode->width << "x" << mode->height << "@" << mode->refreshRate << "Hz" << std::endl;

    std::cout << "UI scaling factor: " << std::fixed << std::setprecision(1) << main_scale << std::endl;
