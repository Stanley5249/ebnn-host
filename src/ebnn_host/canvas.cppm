...
export template <class... Ts> struct Overloads : Ts... {
  using Ts::operator()...;
};
...
public:
  ...
  static ImVec2 getLogicalPos() {
    return Canvas28::SIZE *
           (ImGui::GetIO().MousePos - ImGui::GetItemRectMin()) /
  ...
  void render(ImVec2 img_size) {
    ...
    auto visitor =
        Overloads{[this](StrokeIdle) -> StrokeState {
                  ...
                  [this](StrokeActive state) -> StrokeState {
                    ...
                    bool do_brush = ImGui::IsItemHovered() &&
                                    std::hypot(offset.x, offset.y) > 0.03f;
...
private:
  ...
  static constexpr std::array<std::array<float, KERNEL_SIZE>, KERNEL_SIZE>
      GAUSSIAN_KERNEL{{
